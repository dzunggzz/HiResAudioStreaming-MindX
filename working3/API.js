...
const random404Image = random404Images[Math.floor(Math.random() * random404Images.length)];
...
async function loadTopTracks() {
  ...
  try {
    ...
    const trackPromises = topTracksDatas.tracks.track.map((track) =>
      apiFetch('/search/', { s: `${track.name} ${track.artist.name}`, limit: 1 }).then((res) => res.json()),
    );
...
async function playSongFromQueue(index, forcePlay = false) {
  ...
  try {
    ...
    const streamResponse = await apiFetch('/track/', { id: song.id, quality: quality });
...
function fadeVolume(targetVolume, duration = 300) {
    return new Promise(resolve => {
        ...
        const animate = () => {
            ...
            gainNode.gain.value = startVolume + (targetVolume - startVolume) * progress;
...
function renderLyrics() {
    ...
    currentLyricsLines.forEach((line, index) => {
        ...
        lineEl.className = "lyrics-line py-2 px-4 text-gray-300 hover:text-white cursor-pointer transition-all duration-300 rounded-lg hover:bg-gray-700/50";
